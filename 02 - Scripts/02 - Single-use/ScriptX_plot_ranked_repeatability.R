...
#----------------------------#
temp_repeatability_sort <- "station_count"
...
temp_repeatability_wide <- df_behaviour %>%
  left_join(
    select(df_rec_specialization, animal_id, year, specialization_index_duration),
    by = c("animal_id", "year")
  )

cat("Combined dataset dimensions:", dim(temp_repeatability_wide), "\n")
cat("Unique fish:", n_distinct(temp_repeatability_wide$animal_id), "\n")


### Calculate global rank based on temp_repeatability_sort
#----------------------------#
cat("\n=== CALCULATING GLOBAL RANKS ===\n")
cat("Sorting by:", temp_repeatability_sort, "\n")

temp_rank <- temp_repeatability_wide %>%
  group_by(animal_id) %>%
  summarise(
    mean_sort_value = mean(.data[[temp_repeatability_sort]], na.rm = TRUE),
    .groups = 'drop'
  ) %>%
...
cat("Fish with data for sort metric:", sum(temp_rank$has_data), "\n")
cat("Fish with missing data (ranked at end):", sum(!temp_rank$has_data), "\n")
...
temp_repeatability_long <- temp_repeatability_wide %>%
  left_join(select(temp_rank, animal_id, global_rank), by = "animal_id") %>%
  pivot_longer(
...
cat("Long format dimensions:", dim(temp_repeatability_long), "\n")
cat("Metrics included:", paste(unique(temp_repeatability_long$metric_label), collapse = ", "), "\n")
...
temp_sort_label <- case_when(
  temp_repeatability_sort == "station_count" ~ "Station Count",
  temp_repeatability_sort == "station_count_ratio" ~ "Station Proportion",
  temp_repeatability_sort == "residence_mean" ~ "Residence Duration",
  temp_repeatability_sort == "depth_mean" ~ "Spawning Depth",
  temp_repeatability_sort == "specialization_index_duration" ~ "Specialization Index",
  TRUE ~ temp_repeatability_sort
)

  #  ## REVISION - REVIEWER COMMENT 23 ──────────────────────────
  temp_n_ranked <- length(unique(temp_repeatability_long$global_rank))
  temp_p25 <- temp_n_ranked * 0.25
  temp_p50 <- temp_n_ranked * 0.50
  temp_p75 <- temp_n_ranked * 0.75
  #  ## END REVISION ──────────────────────────────────────────────

plots$behaviour$ranked_repeatability <- temp_repeatability_long %>%
...
temp_summary_by_metric <- temp_repeatability_long %>%
...
temp_fish_means <- temp_repeatability_wide %>%
  group_by(animal_id) %>%
  summarise(
