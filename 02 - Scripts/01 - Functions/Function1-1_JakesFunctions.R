...
MoveInfo <- function(a){
  library(geosphere)
  library(dplyr)
  a <- a %>% arrange(FishID, datetime) %>% as.data.frame()
  p <- a[c("lon", "lat")]
  p1dist <- distGeo(p)
  ...
  p1dist$timediff <- ifelse(p1dist$Transmitter==p1dist$Transmitter2,as.numeric(p1dist$datetime2-p1dist$datetime),60)
...
TimeDiff <- function(a){
  library(dplyr)
  a$nrow <- 1:length(a$datetime)
  p <- data.frame(a)
  p$nrow <- p$nrow+1
  p$datetime2 <- a$datetime[match(p$nrow, a$nrow)]
  p$timediff <- as.numeric(p$datetime2-p$datetime)
  a$timediff <- p$timediff[match(a$nrow, p$nrow)]
  a$timediff[is.na(a$timediff)]<-0
  a$nrow <- NULL
  return(a)
}
...
min_lag <- function (det) 
{
  ...
  dtc[, `:=`(min_lag_system, pmin(diff(c(NA, as.numeric(datetime))), 
